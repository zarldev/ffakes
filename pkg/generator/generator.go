...
func writeAll(w io.StringWriter, igi InterfaceGenInfo) {
	...
	writeHeader(w, igi)
	writeInterfaceConformance(w, igi)
	for _, i := range igi.interfaces {
		writeInterface(w, i)
	}
}

func writeInterfaceConformance(w io.StringWriter, igi InterfaceGenInfo) {
	write(w, "var ( \n")
	for _, iName := range igi.interfaces {
		write(w, fmt.Sprintf("_ %s = (*Fake%s)(nil) \n", iName.Name, iName.Name))
	}
	write(w, ")\n\n")
}
